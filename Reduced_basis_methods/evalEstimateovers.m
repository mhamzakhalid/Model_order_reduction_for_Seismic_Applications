function deltak_LF = evalEstimateovers(s_LF,lam_rand,mu_rand,uk,dqq,Eqq,res_ff,THETA_F_Vec,beta_lb)
Ns_LF = length(s_LF); 
% size 11 x Nz
THETA_A_MAT = zeros(11,Ns_LF);
THETA_A_MAT(1,:) = s_LF.^2;
THETA_A_MAT(2:end,:) = [lam_rand';mu_rand'].*ones(1,Ns_LF);
% 2 real(sum_i=1^11  theta_a*conj(theta_f)*uN'*d_i)     
res_af = real(sum((THETA_A_MAT'.*THETA_F_Vec.') .*(uk(:,:,1)'*dqq)  ,2)).';             
% 2 real(sum_i=1^11  theta_a*theta_a'*uN'*e_ij*uN)             
uk_4d = repmat(uk(:,:,1),[1,1,11,11]);
