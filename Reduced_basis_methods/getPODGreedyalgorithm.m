    % POD-Greedy target algorithm for construction
    
function ROM_PODGreedy = getPODGreedyalgorithm(Aq,bi_HFM,Xh,Rec,s_LF,RICKLF,lam_rand,mu_rand,estimator,var_para,Seismo_HFMTD,Seismo_HFMLD)
    ...
    while Nk<Nkmax && delta_k>epsilontol
        ...
        uh = getPODuh(Aq,bi_HFM,s_LF,RICKLF,lam_rand(maxEstk_idm,:),mu_rand(maxEstk_idm,:));
        
        
        if Nk==1
            POD_basis = POD(Xh,uh,1e-1,1);
            zeta = POD_basis.V;
        else
            POD_basis = POD(Xh,uh - Vk(:,1:Nk-1)*(Vk(:,1:Nk-1)'*(Xh*uh)),1e-1,1);
            zeta = POD_basis.V;
        end
       ...
       %  %%%%%%%%%%%%%% Find RB seismograms and solutions
         tic 
    ...
    ROM_PODGreedy.maxTrueL2m = maxTruekL2m;
    ROM_PODGreedy.maxTruem = maxTruekm;
    ROM_PODGreedy.maxEstimatorm = maxEstkm;
    ROM_PODGreedy.V = Vk;
